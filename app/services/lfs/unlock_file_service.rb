  ...
  class UnlockFileService < BaseService
    ...
    def unlock_file
      ...
      if lock.can_be_unlocked_by?(current_user, forced)
        ...
        error('You must have maintainer access to force delete a lock', 403)
      else
        error("%{lock_path} is locked by GitLab User %{lock_user_id}" % { lock_path: lock.path, lock_user_id: lock.user_id }, 403)
      end
    end
