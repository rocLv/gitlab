...
RSpec.describe Security::Ingestion::IngestReportSliceService do
  ...
  describe '#execute' do
    ...
    before do
      allow(Security::Ingestion::Tasks::IngestIdentifiers).to receive(:execute)
      allow(Security::Ingestion::Tasks::IngestFindings).to receive(:execute)
      allow(Security::Ingestion::Tasks::IngestVulnerabilities).to receive(:execute)
      allow(Security::Ingestion::Tasks::AttachFindingsToVulnerabilities).to receive(:execute)
      allow(Security::Ingestion::Tasks::IngestFindingPipelines).to receive(:execute)
      allow(Security::Ingestion::Tasks::IngestFindingIdentifiers).to receive(:execute)
      allow(Security::Ingestion::Tasks::IngestFindingLinks).to receive(:execute)
      allow(Security::Ingestion::Tasks::IngestFindingSignatures).to receive(:execute)
      allow(Security::Ingestion::Tasks::IngestRemediations).to receive(:execute)
    end
