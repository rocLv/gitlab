...
FactoryBot.define do
  factory :ee_ci_job_artifact, class: '::Ci::JobArtifact', parent: :ci_job_artifact do
    ...
    trait :running_container_scanning do
      file_format { :raw }
      file_type { :running_container_scanning }

      after(:build) do |artifact, _|
        artifact.file = fixture_file_upload(
          Rails.root.join('ee/spec/fixtures/security_reports/master/gl-running-container-scanning-report.json'), 'application/json')
      end
    end
    ...
    trait :running_container_scanning_feature_branch do
      ...
      after(:build) do |artifact, _|
        artifact.file = fixture_file_upload(
          Rails.root.join('ee/spec/fixtures/security_reports/feature-branch/gl-running-container-scanning-report.json'), 'application/json')
      end
    end
