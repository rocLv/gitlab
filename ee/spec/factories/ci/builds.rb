...
FactoryBot.define do
  factory :ee_ci_build, class: 'Ci::Build', parent: :ci_build do
    ...
    %i[api_fuzzing codequality container_scanning running_container_scanning dast dependency_scanning license_scanning performance browser_performance load_performance sast secret_detection coverage_fuzzing].each do |report_type|
    ...
    trait :running_container_scanning_feature_branch do
      after(:build) do |build|
        build.job_artifacts << create(:ee_ci_job_artifact, :running_container_scanning_feature_branch, job: build)
      end
    end

    trait :corrupted_running_container_scanning_report do
      after(:build) do |build|
        build.job_artifacts << create(:ee_ci_job_artifact, :corrupted_running_container_scanning_report, job: build)
      end
    end
