...
RSpec.describe Ci::Pipeline do
  ...
  describe '#security_reports' do
    ...
    before do
      stub_licensed_features(sast: true, dependency_scanning: true, container_scanning: true, running_container_scanning: true)
    end
    ...
    context 'when pipeline has multiple builds with security reports' do
      ...
      let(:build_rcs_1) { create(:ci_build, :success, name: 'rcs_1', pipeline: pipeline, project: project) }
      let(:build_rcs_2) { create(:ci_build, :success, name: 'rcs_2', pipeline: pipeline, project: project) }
      ...
      let!(:rcs1_artifact) { create(:ee_ci_job_artifact, :running_container_scanning, job: build_rcs_1, project: project) }
      let!(:rcs2_artifact) { create(:ee_ci_job_artifact, :running_container_scanning, job: build_rcs_2, project: project) }
      ...
      it 'returns security reports with collected data grouped as expected' do
        expect(subject.reports.keys).to contain_exactly('sast', 'dependency_scanning', 'container_scanning', 'running_container_scanning')
        ...
        expect(subject.get_report('running_container_scanning', rcs1_artifact).findings.size).to eq(2)
      end
